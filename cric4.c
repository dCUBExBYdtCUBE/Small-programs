...
//     char name[18];
//     int playerNumber,wkts,balls,maidens,overs;
//     struct PlayerNode* next;
// } BowlerNode;

// typedef struct BowlerManager {
//     BowlerNode* current;
// } BowlerManager;

// void initializeBowlers(BowlerManager* manager) {
//     manager->current = (BowlerNode*)malloc(sizeof(BowlerNode));
//     if (manager->current == NULL) {
//         printf("Memory allocation failed\n");
//         exit(EXIT_FAILURE);
//     }

//     strcpy(manager->current->name, "Player1");
//     manager->current->playerNumber = 1;

//     BowlerNode* current = manager->current;
//     for (int i = 0; i < 11; i++) {
//         BowlerNode* newPlayer = (BowlerNode*)malloc(sizeof(BowlerNode));
//         if (newPlayer == NULL) {
//             printf("Memory allocation failed\n");
//             exit(EXIT_FAILURE);
//         }

//         // sprintf(newPlayer->name, "Player%d", i+1);
...
//     current->next = manager->current;
// }

// BowlerNode* searchBowler(BowlerManager* manager, const char* targetName, int numBowlers,int t_overs) {
//     BowlerNode* current = manager->current;
//     while(current->next!=current) {
//         if (strcmp(current->name, targetName) == 0) {
// 			if (current->overs/t_overs >= 0.2) {
//                 printf("%s cannot bowl as overs bowled exceed %d\n", targetName,t_overs/current->overs);
//                 return NULL;
//         }
...
void initializePlayers() {
    for (int i = 0; i < MAX_PLAYERS; i++) {
        ...
        players[i].is_on_strike = (i == 0) ? 1 : 0; // Set the first player on strike
    }
}

// void printBowlers(BowlerManager* manager, int numPlayers) {
//     printf("Players in the circular linked list:\n");
...
void updateScore(int runs,int i,int a,int b) {
            players[a].runs_scored += runs;
            players[a].balls_faced++;
            if(runs%2==1){
            players[a].is_on_strike = 0; // Set the current striker off-strike
            players[a].is_on_strike = 1; 
            int temp=a;
            one=b;
            two=temp;
            }// Set the next player on strike
}

void displayPlayerStats() {
    for (int i = 0; i < MAX_PLAYERS; i++) {
		        printf("%s: Runs - %d, Balls Faced - %d\n", players[i].name, players[i].runs_scored, players[i].balls_faced);}
		// if(players[i].balls_faced){
        // printf("%s: Runs - %d, Balls Faced - %d\tStrike Rate - %0.2f\n", players[i].name, players[i].runs_scored, players[i].balls_faced,(players[i].runs_scored/players[i].balls_faced));}
...
int main(int argc, char *argv[]) {
    ...
    // initializeBowlers(&playerManager);
...
void ball_played(int *a,int *b,int *c,int *d,int *e,int *f,int *g,int *h,int *i,int *j){
	...
	switch(runs){
		case '0':
			*a=*a+1;
            for(int i=0; i<11;i++){
                if(players[i].is_on_strike){
                    updateScore(0,i,one,two);
                }
            }
			break;
		case '1':
		...
		case 'O':
			...
			*c=*c+1;
			...
			scanf("%d",&t);
			*b=*b+t;
            if(players[one].is_on_strike){
                players[one].balls_faced+=1;

                one=two+1;
            }
            else if(players[two].is_on_strike){
                players[two].balls_faced+=1;
                two=two+1;
            }
